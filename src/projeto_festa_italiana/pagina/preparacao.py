...
def preparar_pedido(id_pedido: int) -> None:
    # Ler tabelas
    pedidos = ler_pedidos()
    # Alterar tabela
    pedidos = (
...
@st.fragment
def __main__():
    st.title(f"{TITULO} - Conferir e Preparar")
    ct = st.container(border=False)

    pedidos = ler_pedidos().filter(Pago=True, Preparado=False)

    if pedidos.height == 0:
        ct.success("Sem pedidos a preparar")
        return
